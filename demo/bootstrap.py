...
async def bootstrap_demo_pipelines(port: int = 8100) -> None:
    ...
    async with httpx.AsyncClient() as client:
        ...
        # --- Create demo metrics via API + chat ---
        await _bootstrap_demo_metrics(client, base_url, headers, created_pipelines)
...
async def _bootstrap_demo_metrics(
    client: httpx.AsyncClient, base_url: str, headers: dict,
    created_pipelines: list[dict],
):
    """Create demo metrics via REST API + chat interaction.

    Demonstrates the full metrics flow:
    1. Ask the agent to suggest metrics via chat
    2. Create predefined metrics via REST API
    3. Validate via chat asking about trends
    """
    # Idempotency: skip if metrics already exist
    try:
        r = await client.get(f"{base_url}/api/metrics", headers=headers)
        existing = r.json().get("metrics", []) if r.status_code == 200 else []
        if existing:
            log.info("Demo metrics already exist (%d), skipping.", len(existing))
            return
    except Exception:
        pass

    # Find the e-commerce orders pipeline
    orders_pipeline = next(
        (p for p in created_pipelines if "orders" in p.get("pipeline_name", "").lower()),
        None,
    )
    customers_pipeline = next(
        (p for p in created_pipelines if "customers" in p.get("pipeline_name", "").lower()),
        None,
    )

    if not orders_pipeline:
        log.info("No orders pipeline found, skipping demo metrics.")
        return

    orders_pid = orders_pipeline.get("pipeline_id", "")

    # Step 1: Ask the agent to suggest metrics via chat
    log.info("Asking agent to suggest metrics via chat...")
    chat_result = await _create_pipeline_via_chat(
        client, base_url, headers,
        f"Suggest some KPI metrics for the demo-ecommerce-orders pipeline",
        session_id="demo-metrics-suggest",
    )
    if chat_result:
        log.info("Chat metric suggestion routed: action=%s",
                 chat_result.get("routed_action", chat_result.get("action", "?")))

    # Step 2: Also ask via the REST suggest endpoint (exercises agentic suggest)
    try:
        r = await client.post(
            f"{base_url}/api/metrics/suggest/{orders_pid}",
            headers=headers,
            timeout=60.0,
        )
        if r.status_code == 200:
            suggestions = r.json().get("suggestions", [])
            log.info("Agent suggested %d metrics for orders pipeline.", len(suggestions))
        else:
            suggestions = []
            log.warning("Metric suggest returned %d", r.status_code)
    except Exception as e:
        log.warning("Metric suggest failed: %s", e)
        suggestions = []

    # Step 3: Create predefined demo metrics via REST API
    demo_metrics = [
        {
            "pipeline_id": orders_pid,
            "metric_name": "total_orders",
            "description": "Total number of orders in the target table",
            "metric_type": "count",
        },
        {
            "pipeline_id": orders_pid,
    ...
    # Add a customer metric if the pipeline exists
    if customers_pipeline:
        cust_pid = customers_pipeline.get("pipeline_id", "")
        demo_metrics.append({
            "pipeline_id": cust_pid,
            "metric_name": "unique_customers",
            "description": "Count of unique customers loaded",
            "metric_type": "count",
        })
    ...
    for dm in demo_metrics:
        try:
            ...
            if r.status_code == 200:
                ...
                created_metric_ids.append(mid)
                log.info("Created demo metric '%s' (id=%s, sql generated by agent)",
                         dm["metric_name"], mid[:8])
            else:
                log.warning("Create metric '%s' returned %d", dm["metric_name"], r.status_code)
        except Exception as e:
            log.warning("Create metric '%s' failed: %s", dm["metric_name"], e)

    # Step 4: Validate via chat — ask about metrics
    if created_metric_ids:
        chat_result = await _create_pipeline_via_chat(
            client, base_url, headers,
            "What metrics do we have? Can you analyze the trends?",
            session_id="demo-metrics-validate",
        )
        if chat_result:
            log.info("Chat metric validation: %s",
                     chat_result.get("response", chat_result.get("agent_response", ""))[:200])

    log.info("Demo metrics bootstrap complete: %d metrics created.", len(created_metric_ids))
